...
import { getRestaurant } from "../../apis/apis";
...
const FoodDetail = (props) => {
  ...
  const [restaurantDetail, setRestauratDetail] = useState({ name: "" });

  const getRestaurantDetail = async (id) => {
    try {
      const restaurantInfo = await getRestaurant(id);
      setRestauratDetail(restaurantInfo);
    } catch (error) {
      console.log(error);
    } finally {
      ...
      >
        <View style={styles.headerContainer}>
          ...
          <View style={styles.subHeader}>
            <View>
              <Text style={styles.restauarntNameStyle}>
